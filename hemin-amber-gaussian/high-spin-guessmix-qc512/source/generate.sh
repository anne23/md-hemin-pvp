...
# Change SCF options - QC, MaxCycle 512
# Can't find an option to MCPB.py to control this so doing it the hacky way
sed -i '' 's/Opt/SCF=(QC,MaxCycle=512) Opt/g' HEM_small_opt.com
sed -i '' 's/Guess/SCF=(QC,MaxCycle=512) Guess/g' HEM_small_fc.com
sed -i '' 's/Opt/SCF=(QC,MaxCycle=512) Opt/g' HEM_large_mk.com

# Change SCF options - Guess=Mix
# (leave as Read for fc)
# Can't find an option to MCPB.py to control this so doing it the hacky way
sed -i '' 's/Opt/Guess=Mix Opt/g' HEM_small_opt.com
sed -i '' 's/Opt/Guess=Mix Opt/g' HEM_large_mk.com
...
tar -czvf hemin-gaussian-high-spin-guessmix-qc512.tar.gz \
    HEM_small_opt.com \
    HEM_small_fc.com \
    HEM_large_mk.com \
